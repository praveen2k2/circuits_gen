...
class TextToGraphTransformer(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_heads, num_layers, max_seq_len=512, dropout=0.1):
        ...
        self.edge_mlp = nn.Sequential(
            nn.Linear(2 * embedding_dim + 1, hidden_dim),
    ...
    def forward(self, input_seqs, partial_adj=None, return_node_logits=False):
        """
        input_seqs: [B, S]
        partial_adj: [B, S, S] or None — partially known adjacency matrix (0s and 1s)
        """
        ...
        # Add partial adjacency as extra channel
        if partial_adj is not None:
            partial_adj_exp = partial_adj.unsqueeze(-1)  # [B, S, S, 1]
            pairwise = torch.cat([pairwise, partial_adj_exp], dim=-1)  # [B, S, S, 2D + 1]
        else:
            # Default to zeros if not provided
            zeros = torch.zeros(B, S, S, 1, device=x.device)
            pairwise = torch.cat([pairwise, zeros], dim=-1)
        ...
        if return_node_logits:
            node_logits = self.node_classifier(x)  # [B, S, vocab_size]
